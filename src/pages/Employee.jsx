...
function Employee() {
  ...
  const handleEditSubmit = async (updatedEmployee) => {
    ...
    // Validate that the employee has an ID
    if (!updatedEmployee.id) {
      toast.error('Employee ID is missing');
      console.error('Cannot update employee: ID is undefined', updatedEmployee);
      return;
    }
    ...
    try {
      ...
      const cleanedEmployee = {
        ...
        departmentSupervisor: updatedEmployee.departmentSupervisor,
      
        // Address Information
        addressProvince: updatedEmployee.addressProvince,
        addressDistrict: updatedEmployee.addressDistrict,
        addressSector: updatedEmployee.addressSector,
        addressCell: updatedEmployee.addressCell,
        addressVillage: updatedEmployee.addressVillage,
      
        // Emergency Contact
        personOfContactFirstname: updatedEmployee.personOfContactFirstname,
        personOfContactLastname: updatedEmployee.personOfContactLastname,
        personOfContactRelationship: updatedEmployee.personOfContactRelationship,
        personOfContactPhone: updatedEmployee.personOfContactPhone,
      
        // Profile Picture (if applicable)
        passportPicture: updatedEmployee.passportPicture,
      
        // Spread remaining fields to capture any additional data
        ...updatedEmployee
      };

      console.log('Cleaned employee data:', cleanedEmployee);

      const updated = await updateEmployee(cleanedEmployee);
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p>
                <span className={`inline-flex px-2 py-1 text-xs rounded-full ${employee.employee_status === 'Active'
                    ...
                    {filteredData.map((employee) => (
                      <tr key={employee.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3">
                          <span className={`inline-flex px-2 py-1 text-xs rounded-full ${employee.employeeStatus === 'Active'
                              ? 'bg-green-100 text-green-800'
                              : 'bg-red-100 text-red-800'
                            }`}>
                            {employee.employee_status}
